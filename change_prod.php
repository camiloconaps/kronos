...
<script>
function cambiar_prod(id_camp,n_camp){
	swal({
        	title: '¿Esta seguro? ',
                text: n_camp+" cambiara de estado a producción.",
                type: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                confirmButtonText: 'Si, Pasar a producción!',
                cancelButtonText: 'NO'
        }).then(function () {
		$.post( "change_estado.php",{ campid: id_camp, idest:"1" }, function( data ) {
                        if (data == "Error"){
                                $( "#result" ).html( data );
                        }else{
...
<section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Cambio Producción</li>
      </ol>
    </section>
    ...
    <section class="content">
      ...
      <div class="row">
	<?php
        $sql = "SELECT COUNT(*) CNT,ESTADO FROM Campanas WHERE ESTADO = 1 GROUP BY ESTADO";
		...
		// output data of each row
			...
			while($line = $result->fetch_assoc()) {
				...
				echo "<tr class='danger'>";
                                ?>
                                        <td> <input onchange='cambiar_prod("<?php echo$line[ID_CAMPANA]; ?>","<?php echo$line[CAMPANA]; ?>")' type='checkbox' data-toggle='toggle'></td>
                                <?php
				}
				if ($line[ESTADO]==3){
      				echo "<tr class='info'> ";
